...
from .page_branch import PageBranch


##  =====================
##  Branch Viewer Package
##  =====================
...
def _make_page_default(kw):
    ...
    page = PageBranch(default_page, default_template, database_config)
    ...
    msg = (maketext('Branch messages for period'), maketext('Logs For the date'))
    ...
    data_title = '%s%s' % ('', page.has_filter and ' %s [%s]' % (
        maketext('Branch Messages List'), (
            page.shown_filter or '...'
        )) or maketext('Branch Tab Connection Messages Title'))
...
@branch.route('/', methods=['GET', 'POST'])
@branch.route('/index', methods=['GET', 'POST'])
@login_required
def start():
    try:
        return index()
...
def index():
    ...
    return make_response(render_template('branch.html', debug=debug, **kw))


@branch.after_request
...
@branch.route('/loader', methods = ['GET', 'POST'])
@login_required
def loader():
    try:
        return loader_start()
