...
from config import (
     ...
     IsDebug, IsDeepDebug, IsSemaphoreTrace, IsTrace, IsAppCenter, IsAppBranch, LocalDebug, errorlog, print_to, print_exception,
...
def before(f):
    def wrapper(**kw):
        ...
        if default_template == 'none':
            ...
            engine = DatabaseEngine(name=name, connection=CONNECTION[name])
        return f(**kw)
    return wrapper
...
def getCurrentState(template, lid):
    ...
    encode_columns = IsAppCenter and ('Status',) or None
