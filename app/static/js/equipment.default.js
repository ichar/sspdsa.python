...
var $PageController = {
    ...
    activate: function(group, row_id, next_row, chunk) {
        ...
        if (!is_empty(chunk))
            this.tabline.set_chunk(chunk, next_row, 0);
        else
            this.tabline.set_current_by_id(row_id);
        
        var row = this.tabline.get_current();

        if (this.IsLog)
            console.log('main.$PageController.activate.after:', group, row_id, next_row, this.container, row);

        if (!is_exist(row))
            return;

        $PageScroller.refresh(group, 'reset');
    },
    ...
    show_period: function() {
        ...
        content.html(html);
        var top = int(g.top+g.height)-50;
        var left = int(g.left+(g.width/2-p.width/2));

        if (this.IsLog)
            console.log('period:', period, g, p);
