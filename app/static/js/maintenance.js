// *********************************************
// MAINTENANCE PAGE DECLARATION: /maintenance.js
// ---------------------------------------------
// Version: 1.00
// Date: 28-10-2022
...
function activateBatch() {
    ...
    printMaintenancetenenceTZ(area);
...
function activateMaterials(x) {
    ...
    printMaintenancetenenceMaterials(data, props);
...
function activateContainerList(x) {
    ...
    printMaintenancetenenceContainerList(data, props);
}
...
jQuery(function($) 
{
    ...
    $("button[id^='admin']", this).click(function(e) {
        ...
        if (['admin:logsearch'].indexOf(command) > -1) {
            $MaintenancetenenceSubmitDialog.open('logsearch');
            return;
        }
        if (['admin:tagsearch'].indexOf(command) > -1) {
            $MaintenancetenenceSubmitDialog.open('tagsearch');
            return;
        }
    ...
    $("button[id^='action']", this).click(function(e) {
        ...
        if (command == 'action:changedate') {
            $MaintenancetenenceSubmitDialog.open('changedate');
            return;
        }
        if (command == 'action:changeaddress') {
            $MaintenancetenenceSubmitDialog.open('changeaddress');
            return;
        }
    ...
    $(window).keydown(function(e) {
        ...
        if (e.keyCode==13) {                                     // Enter
            ...
            // ------------------------------------
            if ($MaintenancetenenceSubmitDialog.is_focused()) {
                $MaintenancetenenceSubmitDialog.confirmed();
                e.preventDefault();
                return false;
            }
        }

        if ($MaintenancetenenceSubmitDialog.is_focused() || is_search_focused)
            return;
...
function page_is_focused(e) {
    return $MaintenancetenenceSubmitDialog.is_focused();
}
...
{
    if (IsTrace)
        alert('Document Ready (maintenance)');

    current_context = 'maintenance';
